...
class PascalVocWriter:
    ...
    def genXML(self):
        ...
        folder.text = "VOC2007"  # data set name
    ...
    def addBndBox(self, xmin, ymin, xmax, ymax, name, fen):
        bndbox = {'xmin': xmin, 'ymin': ymin, 'xmax': xmax, 'ymax': ymax, 'fen':fen}
        bndbox['name'] = name
    ...
    def appendObjects(self, top):
        for each_object in self.boxlist:
            ...
            obj_value = SubElement(object_item, 'obj_value')
            obj_value.text = str(each_object['fen'])
...
class PascalVocReader:
    ...
    def parseXML(self):
        ...
        size_t = xmltree.find('size')
        width = int(size_t.find('width').text)
        height = int(size_t.find('height').text)
        depth = int(size_t.find('depth').text)
        self.filename = filename
        self.imageInfo.append((width, height, depth))
