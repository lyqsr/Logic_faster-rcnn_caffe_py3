...
from fast_rcnn.config import p_cfg  # loki
...
def _get_image_blob(im, proj_name):
    ...
    IS_3C_GRAY_IMG = p_cfg[proj_name].IS_3C_GRAY_IMG
    ...
    for target_size in SCALES:
        if 0 == Logic_scale_method:
            ...
            im_scale = Logic_scale  # may 1.0 # use fixed image scale first
        ...
        if (not IS_COLOR_IMG) and (not IS_3C_GRAY_IMG):  # loki for (1 channel) gray image
            h, w = im.shape
            im = im.reshape(h, w, 1)
...
def _get_blobs(im, rois, proj_name):
    """Convert an image and RoIs within that image into network inputs."""
    blobs = {'data' : None, 'rois' : None}
    blobs['data'], im_scale_factors = _get_image_blob(im, proj_name)
    if not p_cfg[proj_name].HAS_RPN:
        blobs['rois'] = _get_rois_blob(rois, im_scale_factors)
    return blobs, im_scale_factors

def im_detect(net, im, proj_name, boxes=None):
    ...
    """
    blobs, im_scales = _get_blobs(im, boxes, proj_name)

    # When mapping from image ROIs to feature map ROIs, there's some aliasing
    # (some distinct image ROIs get mapped to the same feature ROI).
    # Here, we identify duplicate feature ROIs, so we only compute features
    # on the unique subset.
    if cfg.DEDUP_BOXES > 0 and not p_cfg[proj_name].HAS_RPN:
    ...
    if p_cfg[proj_name].SVM:
        # use the raw scores before softmax under the assumption they
        # were trained as linear SVMs
        scores = net.blobs['cls_score'].data
    else:
        # use softmax estimated probabilities
        if not (1 == p_cfg[proj_name].Logic_IS_Hingeloss):
            scores = blobs_out['cls_prob']
        # loki use hingeLoss
        else:
            scores = net.blobs['cls_score'].data

    if p_cfg[proj_name].BBOX_REG:
...
def test_net(net, imdb, max_per_image=100, thresh=0.05, vis=False):
    ...
    for i in range(num_images):  # python3
        ...
        if cfg.IS_COLOR_IMG:
            im = cv2.imread(imdb.image_path_at(i), 1) # loki cv2 imread (rgb or bgr) color image
        else:
            im = cv2.imread(imdb.image_path_at(i), 0) # loki cv2 imread gray image
