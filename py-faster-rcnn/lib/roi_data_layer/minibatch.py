...
def _get_image_blob(roidb, scale_inds):
    ...
    for i in range(num_images):  # python3 # xrange
        ...
        if cfg.IS_COLOR_IMG:
            ...
            im = cv2.imread(roidb[i]['image'], cv2.IMREAD_COLOR)  # loki # cv2.imread
        else:
            # loki # gray image
        ...
        im, im_scale = prep_im_for_blob(im, cfg.PIXEL_MEANS, target_size,
                                        cfg.TRAIN.MAX_SIZE)  # loki # lib/utils/blob.py
...
def _vis_minibatch(im_blob, rois_blob, labels_blob, overlaps):
    ...
    for i in range(rois_blob.shape[0]):  # python3 # xrange
        rois = rois_blob[i, :]
        im_ind = rois[0]
        roi = rois[1:]
        ...
        cls = labels_blob[i]
        plt.imshow(im)
        print ('class: ', cls, ' overlap: ', overlaps[i])  # python3 # print
        plt.gca().add_patch(
